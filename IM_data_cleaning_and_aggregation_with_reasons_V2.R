...
build_other_reason_wide <- function(data, reason_col = "other_reason_detail", names_prefix = "other_r_detail_") {
  data %>%
    filter(!is.na(.data[[reason_col]]), .data[[reason_col]] != "") %>%
    mutate(other_reason_slug = make_reason_slug(.data[[reason_col]])) %>%
    count(Country, Region, District, Response, roundNumber, Vaccine.type, other_reason_slug, name = "n") %>%
    pivot_wider(
      names_from = other_reason_slug,
      values_from = n,
      values_fill = 0,
      names_prefix = names_prefix
    )
...
reason_long <- reason_long_main |>
  left_join(
    reason_long_other |>
      select(row_id___, reason_source_col, reason_other_raw),
